...
write_latex_qregnl <- function(reg_obj        # regression object
                            , ys              #  ys for regression
                            , cov_tab         # covariance matrix calculated outside
  ...
  for(i in 1:length(name_var)){
    ...
    anova_mat[i,2] <- sqrt(cov_tab[i,i]) # standard error
